...
public final class EquipmentSurfaceRenderer {
    ...
    public static void reset() { GEOMETRY.clear(); CAPTURED.clear(); captureTick = Long.MIN_VALUE; }
    ...
    public static void modelPart(LivingEntity entity, ItemStack stack, EquipmentSurfaceTarget target,
        ...
        Session session = new Session(entity, stack, target, buffers, light, overlay, armorViewOffset);
    ...
    public static void baked(LivingEntity entity, ItemStack stack, EquipmentSurfaceTarget target,
            List<BakedQuad> quads, PoseStack.Pose pose, MultiBufferSource buffers, int light, int overlay) {
        if (!enabled(entity) || stack.isEmpty()) return;
        Session session = new Session(entity, stack, target, buffers, light, overlay, false);
        if (!session.capture && session.data.isEmpty()) return;
        for (BakedQuad quad : quads) {
            int[] raw = quad.getVertices(); int stride = raw.length / 4;
            EquipmentSurfaceGeometry.Vertex[] vertices = new EquipmentSurfaceGeometry.Vertex[4];
            var sprite = quad.getSprite();
            if (!(sprite.contents() instanceof UploadedSpriteFrame material) || stride < 6) continue;
            for (int i = 0; i < 4; i++) {
                int p = i * stride;
                float u = (Float.intBitsToFloat(raw[p + 4]) - sprite.getU0()) / (sprite.getU1() - sprite.getU0());
                float v = (Float.intBitsToFloat(raw[p + 5]) - sprite.getV0()) / (sprite.getV1() - sprite.getV0());
            ...
            session.face(face, pose, 1F, material, 255);
        }
        session.finish();
    }
    ...
    private static final class Session {
        ...
        Session(LivingEntity entity, ItemStack stack, EquipmentSurfaceTarget target,
                MultiBufferSource buffers, int light, int overlay, boolean armorViewOffset) {
            ...
            capture = entity == minecraft.player && target != null && (entity.tickCount & 1) == 0
        ...
        void face(EquipmentSurfaceGeometry.Face face, PoseStack.Pose pose, float scale, SurfaceMaterial pixels, int vertexAlpha) {
            if (visited >= 8192 || remainingTexels <= 0 || vertexAlpha == 0) return;
            ...
            for (int y = 0; y < face.height() && visited < 8192; y++) for (int x = 0; x < face.width() && visited++ < 8192; x++) {
                ...
                boolean sample = captureFace && Math.floorMod(Long.hashCode(face.id()) + cell, 4) == (entity.tickCount / 2 & 3);
                if (dirty == null && !sample) continue;
                final boolean[] sampled = {false};
                final int px = x, py = y;
                remainingTexels -= SurfaceTexelClip.visit(face,x,y,pixels,remainingTexels,(s0,t0,s1,t1,material) -> {
                    if (sample && !sampled[0]) {
                        sampled[0] = true;
        ...
        void finish() {
            if (capture && !samples.isEmpty()) PacketDistributor.sendToServer(new EquipmentSurfaceContactPayload(
