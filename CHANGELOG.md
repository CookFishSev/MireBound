...
A development snapshot with a new skin editor and improvements from the recent code review.
...
Minecraft 1.21.1 / NeoForge. Internal mod version stays at `0.1.0`; network protocol is now `174`, so clients and servers need the same build. The current source passes 1,214 tests.

## 中文

这次主要加了皮肤沾染编辑器，也把前面代码审查发现的问题整理了一轮，先作为开发快照提交。
...
适用于 Minecraft 1.21.1 / NeoForge。内部模组版本仍为 `0.1.0`，网络协议更新为 `174`，客户端和服务端需要使用同一构建。当前源码通过了 1214 项测试。
